...
const ChatItem = ({ content, role, messageId, onRate }) => {
  ...
  const handleRate = (rating) => {
    if (onRate) {
        onRate(rating);
        axios.post(`http://localhost:5005/api/v1/chat/rate/${messageId}`, { liked: rating === 'like' })
            .then(response => console.log('Rating updated successfully:', response))
  ...
  return role === "assistant" ? (
    ...
    <Avatar sx={{ ml: "0" }}>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", ml: 1 }}>
        <IconButton onClick={() => handleRate("like")}>
          <IoMdThumbsUp />
        </IconButton>
        <IconButton onClick={() => handleRate("dislike")}>
          <IoMdThumbsDown />
        </IconButton>
      </Box>
    </Box>
